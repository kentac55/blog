...
1. ホスト表
    ...
    `kvm`, `k8s-rt`のみfedora33, 他はubuntu20.04
...
1. controller1号機で初期セットアップを行う
    ...
    1. kubeadm initを行う
        ...
        ```bash
        $ sudo kubeadm init --control-plane-endpoint="192.168.21.10:6443" --apiserver-advertise-address="192.168.21.11" --kubernetes-version="v1.20.5" --pod-network-cidr="10.255.0.0/16" --upload-certs | sudo tee -a /etc/kubernetes/kubeadm-init.result
        ```
...
1. k8s構築で作ったUbuntu20.04のVMをLB用に1つクローンする
1. 以下のように作業する
...
1. stats(`http://192.168.21.10:1936`)を見てエラーが出ていないことを確認する

## router構築

metal LBをL3モードで動かすためにBGPを話すrouterを用意する

1. L3SWにログインして192.168.22.0/24のgatewayを192.168.21.20にするstatic routeを追加する(Cisco IOSなら`ip route 192.168.22.0 255.255.255.0 192.168.21.20`とか)
1. 空のVMを作る
1. 適当なfedora33 ISOからfedoraをinstallする
1. クローンする
1. 以下のように作業する
...
1. kubeadm(contextの向き先は`lab1`)が使える環境で以下のファイルを作成する
    ...
    - secret.yml
        ...
        data:
          secretkey: |
            # ここに`openssl rand -base64 128`の結果を入れる
        ```

1. `kustomize build ./ | kubectl apply --server-side -f -`を実行する
    - kubectl v1.21以降は`kubectl -k --server-side -f`でいける([ref](https://github.com/kubernetes/kubernetes/pull/98946))
